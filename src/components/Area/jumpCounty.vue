  ...
  <div class="connect">
    <div class="card_box">
      <el-breadcrumb separator="/">
        ...
        >
        <!-- <el-breadcrumb-item class="currentTitle"
          >区/县级列表</el-breadcrumb-item
        > -->
      </el-breadcrumb>
      <el-card>
        <div class="searchBox">
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <a href="javascript:;" @click="jumpNext(scope.row)">{{
  ...
  methods: {
    ...
    async jumpNext(info) {
      if (info.level == "level-2") {
        this.$router.push("jumpCounty?id=" + info.id+"&level="+info.level);
      } else if (info.level == "level-3") {
        this.$router.push("jumpVillage?id=" + info.id+"&level="+info.level+"&addressP="+info.addressP+"&addressC="+info.addressC+"&addressA="+info.addressA);
      }
    },
    // 新增合作商
    addUsers() {
      var data="?id="+this.$route.query.id+"&level="+this.level;
      if(this.level == "level-2"){
        data+="&addressP="+this.$route.query.addressP+"&addressC="+this.$route.query.addressC
      }else if(this.level == "level-3"){
        data+="&addressP="+this.$route.query.addressP+"&addressC="+this.$route.query.addressC+"&addressA="+this.$route.query.addressA
      }
