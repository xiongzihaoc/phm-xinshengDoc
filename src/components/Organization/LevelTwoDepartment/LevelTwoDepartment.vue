  ...
  <div class="connect">
    <div class="card_box">
      <el-breadcrumb separator="/" v-if="information.level == 'level-5' && information.identify==1">
        ...
        <el-breadcrumb-item class="currentTitle">{{this.$route.query.firstName}}</el-breadcrumb-item>
      </el-breadcrumb>
      <el-breadcrumb separator="/" v-else-if="information.level == 'level-5' && information.identify==2">
        <el-breadcrumb-item class="currentTitle">{{information.org.level2}}列表</el-breadcrumb-item>
      </el-breadcrumb>
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <a
                href="javascript:;"
                @click="jumpLevelThreeDepartment(scope.row)"
...
export default {
  ...
  methods: {
    // 获取机构列表
    async getScaleList() {
      const { data: res } = await this.$http.post("dealer/getSecondDept", {
        id: parseInt(this.id),
    ...
    showEditdialog(info) {
      this.$router.push(
        "AddSecondDepartment?orgId=" +
          ...
          "&isEditType=1" +
          "&id=" +
          info.id+"&org="+this.$route.query.org
      );
    },
    // 修改密码
    revampPassword(info) {
    ...
    // 点击名称跳转下一级
    jumpLevelThreeDepartment(info) {
      var orgName = this.$route.query.orgName;
      var orgId = this.$route.query.orgId;
      var firstName = this.$route.query.firstName;
      var firstId = this.$route.query.id;
      this.$router.push(
        "LevelThreeDepartment?id=" +
          ...
          firstName +
          "&firstId=" +
          firstId+"&org="+this.$route.query.org
      );
    },
    // 删除
    async deleteBtn(info) {
      ...
      const { data: res } = await this.$http.post("dealer/secondDept/del", {
        id: info.id,
        firstId: this.id,
      });
