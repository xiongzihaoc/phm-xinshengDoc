    ...
    revampPasswordEnter() {
      if (this.addForm.submitPassword != this.addForm.newPassword) {
        ...
        this.$refs.addFormRef.validate(async (valid) => {
          if (!valid) return;
          var portName = "dealer/updatePwd"; // 默认机构修改密码接口
          if (this.type == "firstDept") {
            // 一级部门修改密码
            portName = "dealer/firstDept/updatePwd";
          } else if (this.type == "secondDept") {
            // 二级部门修改密码
            portName = "dealer/secondDept/updatePwd";
          } else {
            portName = "dealer/updatePwd";
          }
          const { data: res } = await this.$http.post(portName, {
            id: this.addForm.id,
