  ...
  <div class="connect">
    <div class="card_box">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item class="currentTitle">预警列表</el-breadcrumb-item>
      </el-breadcrumb>
      <el-card>
        <div class="searchBox">
          <el-input
...
export default {
  ...
  methods: {
    // 获取列表
    async getScaleList() {
      const { data: res } = await this.$http.post("dealer_check/warn_list", {
    ...
    lookOver(val) {
      console.log(val);
      this.$router.push("WarningDetail?id=" + val.id + "&idCard=" + val.idCard);
    },
    // 搜索
    searchin() {
      this.getScaleList();
